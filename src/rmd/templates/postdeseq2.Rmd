...
if (type_ids %in% c("ensembl_gene_id", "hugo_symbol") ) {
  urls$gprofiler$dereg <- gost(
    query = r_signif[, type_ids],
    as_short_link = TRUE
  )
  urls$gprofiler$upreg <- gost(
    query=r_signif[r_signif$log2FoldChange > 0, type_ids],
    as_short_link = TRUE
  )
  ...
  urls$gprofiler$multiquery <- gost(
    query = list(
      dereg = r_signif[, type_ids],
      upreg = r_signif[r_signif$log2FoldChange > 0, type_ids],
      downreg = r_signif[r_signif$log2FoldChange < 0, type_ids]
    ),
    as_short_link = TRUE
  )
  ...
  cat(
    ...
    ")\n* g:Profiler results for upregulated genes are available [here](",
    urls$gprofiler$upreg,
    ")\n* g:Profiler results for downregulated genes are available [here](",
