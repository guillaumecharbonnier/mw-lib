...
```{r {{chunk_label_prefix}}CheckGoodSamples}
gsg <- goodSamplesGenes(t(assay(d)), verbose = 0)

if (gsg$allOK) {
  check_good_samples <- "All samples and genes look good for the analysis."
} else {
  check_good_samples <- "At least some samples or genes look not suitable for the analysis."
}
...
There are `r nrow(d)` genes and `r ncol(d)` samples in the dataset for network creation. `r check_good_samples`
...
tmp <- data.table(
  Gene_name = rowData(d)$external_gene_name,
...
```{r}
message("{{chunk_label_prefix}}EnrichPathway")
```
...
if ("{{chunk_label_prefix}}" == 
"resultsWgcnaTcgaSarcMalignantFibrousHistiocytomaManualPower9Knit") {
  #browser()
}
...
msigdb <- read.gmt(file.path(mw_from_rmd, "../mw-ibd/inp/h.all.v7.0.entrez.gmt"))
...
if (nrow(d_subset) > 2000) {
  ...
  p <- Heatmap(
    assay(d_subset),
    row_labels = rowData(d_subset)$external_gene_name,
    km = 2,
    name = "RNA\nVST\nsignal",
...
```{r {{chunk_label_prefix}}PlotGenesInVnn1Module}
...
```{r {{chunk_label_prefix}}PrintGenesInVNN1Module}
message(opts_current$get("label"))
if (nrow(d_subset) > 5000) {
} else {
  produceDataTableWithButtons(
    data.frame(
      cor_vs_VNN1 = cor_subset[,"ENSG00000112299"],
      rowData(d_subset)
    )
...
```{r {{chunk_label_prefix}}relateModulesToMetadataBicorUnsignedMergeCutHeight0Point15Power13}

# MEs0 <- moduleEigengenes(t(assay(d)), moduleColors)$eigengenes
# MEs <- orderMEs(MEs0)

# datTrait <- vst_data$colData
# datTrait$sex <- as.numeric(factor(datTrait$sex))
# datTrait$smoking.bool <- as.numeric(factor(datTrait$smoking.bool))
# # browser()
# moduleTraitCor <- cor(MEs, datTrait, use = "p")
# moduleTraitPvalue <- corPvalueStudent(moduleTraitCor, nrow(vst_data$colData))
...
```{r {{chunk_label_prefix}}plotHetmapRelationsModulesToMetadata, fig.cap=fig_cap, fig.width=fig_width, fig.height=fig_height}
# fig_width <- 3 + ncol(moduleTraitCor) * 0.25
# fig_height <- 3 + nrow(moduleTraitCor) * 0.25
# fig_cap <- "Pearson correlation between module eigengene and metadata"

# # par(mar = c(6, 8.5, 3, 3))
# # Display the correlation values within a heatmap plot
# labeledHeatmap(
#   Matrix = moduleTraitCor,
#   xLabels = names(vst_data$colData),
#   yLabels = names(MEs),
#   ySymbols = names(MEs),
#   colorLabels = FALSE,
...
```{r {{chunk_label_prefix}}preparePlotHeatmapSelectedModule, eval=FALSE}
p_basename <- file.path(
  book_from_rmd,
  "plots",
  opts_current$get("label")
)

p_basename_absolute <- paste0(p_basename, "/AbsoluteValues/")
p_basename_zscore <- paste0(p_basename, "/ZscoreValues/")
p_basename_zscore_kmeans2 <- paste0(p_basename, "/ZscoreValues_kmeans2/")

dir.create(p_basename_absolute, recursive = TRUE)
dir.create(p_basename_zscore)
dir.create(p_basename_zscore_kmeans2)

# No grey because it is too big for RAM
selected_modules <- unique(tmp$module)
selected_modules <- selected_modules[selected_modules != "grey"]

for (selected_module in selected_modules) {
  cat("Enrichments analyses for ", selected_module, " module")
  selected_module_data <- assay(d)[which(tmp$module == selected_module), ]
  selected_module_gene_name <- vst_data$rowData$Gene_name[which(tmp$module == selected_module)]
  selected_module_entrez_id <- vst_data$rowData$entrez_id[which(tmp$module == selected_module)]

  # fig_height <- min(49, 1 + 0.3*nrow(selected_module_data))
  # fig_cap <- 'log2(vst) signal in VNN1 module. Rows were gene name is prefixed with "-" display negative Z-score for these genes, meaning their signal is anticorrelated with other genes in the cluster. [pdf version with readable genes and findable VNN1](plots/plotHeatmapVnn1ModuleBicorAbsoluteValues.pdf)'

  p <- pheatmap(
    mat = selected_module_data,
    # annotation_col = to_display_data$colData[,interesting_metadata],
    main = paste(selected_module, "module, absolute VST-normalized values"),
    cluster_cols = FALSE,
    labels_row = selected_module_gene_name,
    fontsize = 8
  )
  p
  ...
  ggsave(
    filename = p_filename, p, limitsize = FALSE,
    height = 1 + 0.28 * nrow(selected_module_data)
  )

  selected_module_data <- t(scale(t(selected_module_data)))

  p <- pheatmap(
    mat = selected_module_data,
    # annotation_col = to_display_data$colData[,interesting_metadata],
    main = paste(selected_module, "module, Zscore of VST-normalized values"),
    cluster_cols = FALSE,
    labels_row = selected_module_gene_name,
    fontsize = 8
  )
  ...
  entrez_gene_list <- list()
  entrez_gene_list$cluster_1 <- na.omit(selected_module_entrez_id[p$kmeans$cluster == 1])
  entrez_gene_list$cluster_2 <- na.omit(selected_module_entrez_id[p$kmeans$cluster == 2])
  print(paste("Cluster 1 can be associated with", length(entrez_gene_list$cluster_1), "genes with Entrez ID"))
  print(paste("Cluster 2 can be associated with", length(entrez_gene_list$cluster_2), "genes with Entrez ID"))

  res <- try(
    {
      compareCluster(
        geneCluster = entrez_gene_list,
        fun = "enricher",
        TERM2GENE = msigdb,
        universe = unique(na.omit(vst_data$rowData$entrez_id))
      )
    },
    silent = TRUE
  )
  ...
  if ("try-error" %in% class(res)) {
    print("No enriched Hallmarks term in the 2 kmeans clusters of this module")
  } else {
    p <- dotplot(res,
      title = paste(selected_module, "module, Hallmarks for the 2-means clusters"),
      showCategory = 30
    )
    p <- p + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
    print(p)
  ...
  res <- try(
    {
      compareCluster(
        geneCluster = entrez_gene_list,
        fun = "enrichKEGG",
        title = paste(selected_module, "module, KEGG for the 2-means clusters"),
        pvalueCutoff = 0.05,
        pAdjustMethod = "BH"
      )
    },
