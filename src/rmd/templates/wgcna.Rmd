...
fig_cap <- "Genes in VNN1 module plotted by Pearson correlation against it."
...
if (nrow(d_subset) > 5000) {
  ...
  produceDataTableWithButtons(r)

  urls <- list()
  if ("hugo_symbol" %in% colnames(r)) {
    urls$enrichr$positive_correl_with_VNN1 <- getEnrichrUrlFromGenes(r[r$cor_vs_VNN1 > 0,]$hugo_symbol)
    urls$enrichr$negative_correl_with_VNN1 <- getEnrichrUrlFromGenes(r[r$cor_vs_VNN1 < 0,]$hugo_symbol)
  ...
  if (type_ids %in% c("ensembl_gene_id", "hugo_symbol") ) {
    ...
    urls$gprofiler$positive_correl_with_VNN1 <- gost(
      query=r[r$cor_vs_VNN1 > 0, type_ids],
      as_short_link = TRUE
    )
    ...
    urls$gprofiler$multiquery <- gost(
      query = list(
        dereg = r[, type_ids],
        upreg = r[r$cor_vs_VNN1 > 0, type_ids],
        downreg = r[r$cor_vs_VNN1 < 0, type_ids]
      ),
      as_short_link = TRUE
    )
