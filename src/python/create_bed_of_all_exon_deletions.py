...
# Add a strand-specific treatment to get proper exon numbering
# Adapted from:
# https://github.com/Oshlack/Toblerone/blob/master/scripts/create_bedfiles.py
...
def get_internals(combo):
    ...
    for i in range(1,len(combo)):   # for the length of the full sequence
        for y in range(1,i):
            ...
            #print(mask)
            subs.append(combo[np.isin(combo, mask, invert=True)])
    
    return(subs)


def make_bed_file(lengths, positions, orig_exon, seed_bed1, seed_bed2, bedfilename, strand, gapsallowed = False):
    ...
    for sub in all_subsets_v2:
        ...
        # Rename the exons for negative strand genes
        if strand == '-':
            total_exons = len(lengths_lookup)
            del_name = [str(total_exons - int(x) + 1) for x in del_name]
            # Reorder del_name numerically
            del_name = sorted(del_name, key=int)
...
for k,v  in full_gene_dict.items():
    ...
    make_bed_file(lengths, positions, orig_exon, seed_bed1, seed_bed2, bedfilename, strand, gapsallowed = False)
