...
rule bedtools_multiinter_thymus_peaks_hg38_merged:
    """
    Created:
        2018-03-19 10:46:58
    Aim:
    ...
    input:
        ...
        bed_sp8  = "out/sort/coordinates_bed/macs2/callpeak_format-BAM_gsize-hs_nomodel/ln/alias/experiments/hg38_H3K27ac_thymus/SP8_over_input_peaks.narrowPeak"
    output:
        #bed_combination= expand("out/bedtools/multiinter_thymus_peaks_hg38_merged/{combination}.bed", combination=MULTIINTER_COMBINATION),
        bed_multiinter = "out/bedtools/multiinter_thymus_peaks_hg38_merged/multiinter.bed",
        bed_all        = "out/bedtools/multiinter_thymus_peaks_hg38_merged/all_f0.5.bed",
        bed_combi      = expand("out/bedtools/multiinter_thymus_peaks_hg38_merged/{combination}.bed", combination=BEDTOOLS_MULTIINTER_THYMUS_PEAKS_HG38_MERGED_COMBINATION)
        #bed_not_common = "out/bedtools/multiinter_thymus_peaks_hg38_merged/not_common.bed",
        #bed_cd34       = "out/bedtools/multiinter_thymus_peaks_hg38_merged/CD34.bed",
        #bed_ec         = "out/bedtools/multiinter_thymus_peaks_hg38_merged/EC.bed",
        #bed_lc         = "out/bedtools/multiinter_thymus_peaks_hg38_merged/LC.bed",
    ...
    shell:
        """
        {input.bedtools} multiinter -i {input.bed_cd34} {input.bed_ec} {input.bed_lc} {input.bed_sp4} {input.bed_sp8} > {output.bed_multiinter}
        {input.bedtools} intersect -a {input.bed_all} -b {output.bed_multiinter} -wa -wb -f 0.5 > {output.bed_all}

        for CLASS in `cut -f8 {output.bed_all} | sort -u`
        do
            echo $CLASS
            awk -v CLASS="$CLASS" 'BEGIN {{OFS="\\t"}}; ($8 == CLASS) {{print $0}}' {output.bed_all} > {params.outdir}/$CLASS.bed
        done
        """

rule bedtools_multiinter_bs_hypometh_thymus:
    """
    Created:
        2018-04-23 15:31:26
    Aim:
        Testing if hypomethylated regions can be used as reference to compare dynamics in histone marks.
    """
    input:
        bed = expand("out/gunzip/to-stdout/ln/updir/mw-thymus/inp/TAGC_Necker/{sample}.bed", sample=BLUEPRINT_BED_BS_HYPOMETH)
    output:
        bed_multiinter = "out/bedtools/multiinter_bed-hg38-bs-hypometh-thymus/multiinter.bed",
        bed_common_all = "out/bedtools/multiinter_bed-hg38-bs-hypometh-thymus/common_all.bed"
    conda:
        "../envs/bedtools.yaml"
    shell:
        """
        OUTDIR=`dirname {output.bed_multiinter}`

        bedtools multiinter -i {input.bed} > {output.bed_multiinter}

        awk 'BEGIN {{OFS="\\t"}}; ($5 == "1,2,3,4,5,6,7,8,9") {{print $1,$2,$3}}' {output.bed_multiinter} > {output.bed_common_all}
        """
