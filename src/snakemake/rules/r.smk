...
rule r_mca:
    ...
    Created:
        2018-05-03 14:28:34
    Aim:
        A script to take Chromdet samples_collapsed_filtered.tab and compute MCA on it.
    Test:
        out/r/mca/alternative-collapse_segments-hg19-segments-our-complete-merged-thymic-samples-into-chromdet-original-paper/fviz_mca_ind.pdf        out/r/mca/alternative-collapse_segments-hg19-segments-our-complete-merged-thymic-samples-into-chromdet-original-paper-t-cells/fviz_mca_ind.pdf out/r/mca/alternative-collapse_segments-hg19-segments-our-complete-merged-thymic-samples-into-chromdet-original-paper-mhsc-t-cells/fviz_mca_ind.pdf
        out/r/mca/segments-hg19-segments-our-complete-merged-thymic-samples-into-chromdet-original-paper/fviz_mca_ind.pdf
        out/r/mca/segments-hg19-segments-our-complete-merged-thymic-samples-into-chromdet-original-paper-t-cells/fviz_mca_ind.pdf
        out/r/mca/segments-hg38-segments-our-complete-thymic-samples-into-chromdet-original-paper/fviz_mca_ind.pdf
        out/r/mca/segments-hg38-segments-our-thymic-samples-into-chromdet-original-paper/fviz_mca_ind.pdf
        out/r/mca/redo_original_paper_table_s1/fviz_mca_ind.pdf
        out/r/mca/redo_original_paper_table_s1_with_disease/fviz_mca_ind.pdf
    """
    input:
        #rscript = "opt/miniconda/envs/rca/bin/Rscript",
        code = "src/r/script/mca.R",
        tab  = "out/chromdet/{filler}/samples_collapsed_filtered.tab"
    output:
        expand("out/r/mca/{{filler}}/{files}",files=["mca.Rdata","fviz_screeplot.pdf","fviz_mca_ind.pdf","fviz_mca_ind_14x14.pdf"])
    params:
        outdir="out/r/mca/{filler}"
    conda:
        "../envs/r_component_analysis.yaml"
    shell:
        "Rscript {input.code} -t {input.tab} -o {params.outdir}"

rule r_thymus_article_mca_draft:
    input:
        data = "out/r/mca/segments-bed-hg19-segments-our-complete-merged-thymic-samples-into-chromdet-original-paper/mca.Rdata",
        #rscript = "opt/miniconda/envs/rca/bin/Rscript",
        code = "src/r/script/thymus_article_mca_draft.R"
    output:
        pdf="out/r/thymus_article_mca_draft/fviz_mca_ind_arrange_7x14.pdf"
    conda:
        "../envs/r_component_analysis.yaml"
    shell:
        "Rscript {input.code}"
...
rule r_pca_cpg:
    ...
    Created:
        2018-05-11 09:47:35
    Aim:
        Plot PCA for level of methylation of CpG in our thymic samples.
    Test:
        out/r/pca_cpg/fviz_eig.pdf
    """
    input:
        code = "src/r/script/pca_cpg.R",
        tab  = expand("out/gunzip/to-stdout/ln/alias/experiments/thymus_BS/{sample}.txt", sample=BLUEPRINT_THYMUS_BS_CPG)
    output:
        expand("out/r/pca_cpg/{files}",files=["fviz_eig.pdf","fviz_pca_ind.pdf","fviz_pca_ind_axes_2_3.pdf"])
    conda:
        "../envs/r_component_analysis.yaml"
    shell:
        """
...
rule r_pca_cpg_bw_blueprint_healthy:
    """
    Created:
        2018-05-11 09:47:35
    Aim:
        Plot PCA for level of methylation of CpG in our thymic samples.
    Test:
        out/r/pca_cpg_bw-hg38-bs-blueprint-hmpc-t-methylation-calls/fviz_eig.pdf
    """
    input:
        code = "src/r/script/pca_cpg_bw.R",
        #bw  = expand("out/ln/alias/experiments/BS_Blueprint_healthy_methylation_calls/{sample}.bw", sample=BS_BLUEPRINT_HEALTHY_METHYLATION_CALLS),
        bw_list = lambda wildcards: eval(config['ids'][wildcards.bw_list_id]),
        rmsk= "out/awk/extract_bed6_from_rmsk/gunzip/to-stdout/wget/http/hgdownload.cse.ucsc.edu/goldenpath/hg38/database/rmsk.bed"
    output:
        pdf=expand("out/r/pca_cpg_{{bw_list_id}}/{files}",files=["fviz_eig.pdf","fviz_pca_ind.pdf","fviz_pca_ind_axes_2_3.pdf"])
    conda:
        "../envs/r_component_analysis.yaml"
    shell:
        """
        FIRSTPDF=`echo "{output.pdf}" | cut -f1 -d' '`
        OUTDIR=`dirname $FIRSTPDF`
        BWLIST=`echo {input.bw_list} | sed 's/ /,/g'`
        Rscript {input.code} -r {input.rmsk} -o $OUTDIR -b $BWLIST
        """
