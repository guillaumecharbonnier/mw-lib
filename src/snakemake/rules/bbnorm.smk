rule bbnorm_se:
    ...
    Test:
        out/bbnorm/se/ln/alias/sst/all_samples/fastq/802_BT11_H3K27ac.fastq.gz
    """
    input:
    ...
    conda:
        "../envs/bbmap.yaml"
    shell:
        "bbnorm.sh {params.extra} in='{input.fq}' out='{output.fq_to_keep}' &> {log}"
...
rule bbnorm_pe:
    ...
    IUse in2 for paired reads in a second file  
    Test:
        out/bbnorm/pe/ln/alias/sst/all_samples/fastq/856_H3K27ac_1.fastq.gz
    """
    input:
    ...
    conda:
        "../envs/bbmap.yaml"
    shell:
       "bbnorm.sh {params.extra} in='{input.fq_1}' in2='{input.fq_2}' out='{output.fq_1}' out2='{output.fq_2}' &> {log}"
